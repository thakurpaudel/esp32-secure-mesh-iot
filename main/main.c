...
//     ESP_LOGW(TAG, "<MESH_EVENT_ROUTING_TABLE_CHANGE>new:%d, remove:%d",
//              routing_table->rt_size_new, routing_table->rt_size_change);
//   } break;
//   case MESH_EVENT_NO_PARENT_FOUND:
//     ESP_LOGI(TAG, "<MESH_EVENT_NO_PARENT_FOUND>");
//     /* TODO handler for the failure */
//     break;
//   case MESH_EVENT_PARENT_CONNECTED: {
//     mesh_event_connected_t *parent_connected =
...
//              "%s, ID:" MACSTR "",
//              last_layer, layer, MAC2STR(parent_connected->connected.bssid),
//              esp_mesh_is_root() ? "<ROOT>" : "", MAC2STR(id.addr));
//     last_layer = layer;
//     is_mesh_connected = true;
//   } break;
//   case MESH_EVENT_PARENT_DISCONNECTED: {
//     mesh_event_disconnected_t *parent_disconnected =
//         (mesh_event_disconnected_t *)event_data;
//     ESP_LOGI(TAG, "<MESH_EVENT_PARENT_DISCONNECTED>reason:%d",
//              parent_disconnected->reason);
//     is_mesh_connected = false;
//   } break;
//   case MESH_EVENT_LAYER_CHANGE: {
//     // mesh_event_layer_change_t *layer_change = (mesh_event_layer_change_t
//     // *)event_data;
//     int layer = esp_mesh_get_layer();
...
//     last_layer = layer;
//     // Layer change doesn't necessarily mean disconnected, but let's
//     re-verify
//     // properly in robust logic. However, PARENT_CONNECTED sets true,
//     // PARENT_DISCONNECTED sets false. If we become root, we might not get
//     // PARENT_CONNECTED? MESH_EVENT_ROOT_GOT_IP is better for root, but for
//     // simple fix:
//     if (esp_mesh_is_root())
//       is_mesh_connected = true;
//   } break;
...
//   mesh_data_t data;
//   data.data = (uint8_t *)"Hello Mesh";
//   data.size = sizeof("Hello Mesh");
...
//     // Only send if not root (Root receives)
//     if (esp_mesh_is_root()) {
//       // Root doesn't send to itself in this demo
//       ESP_LOGI(TAG,
//                "I am ROOT (Layer 1). Waiting for messages from children...");
//       vTaskDelay(5 * 1000 / portTICK_PERIOD_MS);
//       continue;
//     }

//     esp_mesh_get_routing_table((mesh_addr_t *)&route_table, 6 * 6,
//                                &route_table_size);
...
//     // Send to Root (using MESH_DATA_TODS flag is simplest for node->root)
//     // or we can just target the root address if we know it.
//     // However, esp_mesh_send can send to external IP if MESH_DATA_TODS is
//     used.
//     // For Node-to-Root internal P2P, we can fetch root address or just send
//     to
//     // parent? Let's use MESH_DATA_TODS which goes to root.
//     err = esp_mesh_send(NULL, &data, MESH_DATA_TODS, NULL, 0);

//     if (err) {
//       ESP_LOGE(TAG, "Failed to send message: %d", err);
//     } else {
//       ESP_LOGI(TAG, "Sent message %d to ROOT", i++);
//     }
...
//   int flag = 0;
//   mesh_data_t data;
//   uint8_t *data_buf = (uint8_t *)malloc(1500); // Buffer for RX data
...
//   }
//   data.data = data_buf;
//   data.size = 1500; // Max size

//   while (1) {
//     data.size = 1500; // Reset size for each recv
...
//     ESP_LOGI(TAG, "[DATA] Received %d bytes from " MACSTR ": %s", data.size,
...
//   // Set Wi-Fi storage to RAM to avoid NVS spam
...
//   mesh_cfg_t mesh_cfg = {
//       .channel = 0, // 0 = Auto-scan for channel
//       .router =
//           {
//               .ssid = "INCOGNITO",
//               .password = "Ronaldo@728",
//               .ssid_len = sizeof("INCOGNITO") - 1,
//           },
...
void mesh_scan_done_handler(int num) {
  ...
  for (i = 0; i < num; i++) {
    ...
    if (ie_len == sizeof(assoc)) {
      ESP_LOGW(MESH_TAG,
               "<MESH>[%d]%s, layer:%d/%d, assoc:%d/%d, %d, " MACSTR
               ", channel:%u, rssi:%d, ID<" MACSTR "><%s>",
      ...
      ESP_LOGI(MESH_TAG, "[%d]%s, " MACSTR ", channel:%u, rssi:%d", i,
               record.ssid, MAC2STR(record.bssid), record.primary, record.rssi);
  ...
  if (parent_found) {
    ...
    if (my_type == MESH_ROOT) {
      ...
      ESP_LOGW(MESH_TAG,
               "<PARENT>%s, layer:%d/%d, assoc:%d/%d, %d, " MACSTR
               ", channel:%u, rssi:%d",
               parent_record.ssid, parent_assoc.layer, parent_assoc.layer_cap,
               parent_assoc.assoc, parent_assoc.assoc_cap,
               parent_assoc.layer2_cap, MAC2STR(parent_record.bssid),
               parent_record.primary, parent_record.rssi);
      ESP_ERROR_CHECK(esp_mesh_set_parent(
          &parent, (mesh_addr_t *)&parent_assoc.mesh_id, my_type, my_layer));
    }
...
void app_main(void) {
  ...
  ESP_ERROR_CHECK(
      esp_mesh_set_ie_crypto_funcs(&g_wifi_default_mesh_crypto_funcs));
  ESP_ERROR_CHECK(esp_mesh_set_ie_crypto_key(
      CONFIG_MESH_IE_CRYPTO_KEY, strlen(CONFIG_MESH_IE_CRYPTO_KEY)));
#else
  /* disable IE crypto */
